...
static bool battery_recharge_cond(struct battery_info *info)
{
	...
	ktime_t recharge_ktime;
	struct timespec recharge_current_time;

	recharge_ktime = alarm_get_elapsed_realtime();
	info->recharge_current_time = recharge_current_time = ktime_to_timespec(recharge_ktime);
#endif
	pr_debug("%s\n", __func__);
...
#if defined(CONFIG_MACH_BAFFIN_KOR_LGT)	
	pr_info("%s: recharge state1, vcell(%d ? %d)\n", __func__,
		info->battery_vcell, info->pdata->recharge_voltage);
	if ((info->battery_vcell < info->pdata->recharge_voltage) && (info->recharge_count == 0)) {
		info->recharge_start_time = recharge_current_time.tv_sec;
		info->recharge_count++ ;
	}
	
	if (info->recharge_count == 1){
		pr_info("%s: recharge timer state, t(%d - %d ?? 145)\n", __func__,
			(int)recharge_current_time.tv_sec, info->recharge_start_time);
		if((recharge_current_time.tv_sec - info->recharge_start_time) > 145) {
			pr_info("%s: recharge state2, vcell(%d ? %d)\n", __func__,
				info->battery_vcell, info->pdata->recharge_voltage);
					...
					union power_supply_propval *val)
{
	...
	#if defined(CONFIG_MACH_GC1) || defined(CONFIG_MACH_SUPERIOR_KOR_SKT)
		info->psy_fuelgauge->set_property(info->psy_fuelgauge,
