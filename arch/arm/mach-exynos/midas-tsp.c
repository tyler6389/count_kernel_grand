...
#elif defined(CONFIG_TOUCHSCREEN_CYPRESS_TMA46X)
static DEFINE_SPINLOCK(led_control_lock);
...
int cyttsp4_led_power(int on)
{
	...
	if (on) {
		...
		spin_lock_irqsave(&led_control_lock, flags);

		for (; max_lev > led_lev; max_lev--) {
			udelay(1);
		...
		spin_unlock_irqrestore(&led_control_lock, flags);
